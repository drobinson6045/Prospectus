...
\begin{center}
\includegraphics[width=0.6\textwidth]{C:/Users/David/FSU/Manna/DomainSchem}
\par\end{center}
...
\caption{Snapshots of two moving interfaces initialized with the same shape
and forcing. The left interface has no tangential velocity component
and the right interface has the tangential velocity implemented {[}2{]}.
Notice that the right interface has the same shape as the left and
keeps points equispaced in arclength.\label{lTheta}}
\end{wrapfigure}%
...
\bibliography{C:/Users/David/References}
